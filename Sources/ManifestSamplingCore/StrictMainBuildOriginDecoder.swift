...
enum StrictMainBuildOriginDecoder {
  static func decode(
    _ body: Data,
    ...
    let decoded: MainBuildOriginRootDTO
    do {
      decoded = try JSONDecoder().decode(MainBuildOriginRootDTO.self, from: owned)
    } catch {
      throw ManifestSamplingError.semanticShapeRejected
    }
    ...
    try cancellationCheck()
    return ValidatedMainBuildOriginReference(
  ...
  private static func makeBinding(
    ...
    var framed = Data("MGB_CN_MAIN_BUILD_ORIGIN_REFERENCE\0".utf8)
...
struct ValidatedMainBuildOriginReference: Equatable, Sendable {
...
extension ValidatedMainBuildOriginReference: ManifestSamplingRedactedValue {}

private struct MainBuildOriginRootDTO: Decodable {
  let retcode: Int
  let data: MainBuildOriginDataDTO
}

private struct MainBuildOriginDataDTO: Decodable {
  let tag: String
  let manifests: [MainBuildOriginManifestDTO]
}

private struct MainBuildOriginManifestDTO: Decodable {
  let matchingField: String
  let manifestDownload: MainBuildOriginDownloadDTO
...
private struct MainBuildOriginDownloadDTO: Decodable {
